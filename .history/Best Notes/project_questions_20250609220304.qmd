...
These fields don’t contain the Coverage or Practitioner details — they just point to them."

Should our code be responsible for resolving these references by loading the associated JSON files (e.g., Coverage/example → FHIR-Coverage-example.json)?

Or...

Will we always be given a complete, pre-resolved input (e.g., a single big JSON file or in-memory array that includes everything)?
